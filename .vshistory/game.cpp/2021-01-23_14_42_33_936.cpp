...
void Game::rocketsFunc() {
	for (Rocket& rocket : rockets)
	{
		...
		K?rockets.erase(std::remove_if(rockets.begin(), rockets.end(), [](const Rocket& rocket) { return (!rocket.active || rocket.position.x < 0 || rocket.position.x > SCRWIDTH || rocket.position.y > SCRHEIGHT); }), rockets.end());
