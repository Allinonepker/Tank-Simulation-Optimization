...
void Game::update(float deltaTime)
{
	...
	thread C(thread(thread(collisionTanks)));
	thread S(thread(thread(smokeFunc)));
	thread R(thread(thread(rocketsFunc)));
	thread P(thread(thread(particleBeam)));
	thread E(thread(thread(explosionsFunc)));
	//collisionTanks();
	//smokeFunc();
	//rocketsFunc();
	//particleBeam();
