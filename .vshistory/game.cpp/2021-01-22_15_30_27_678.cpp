...
vector<Tank&> Tmpl8::Game::merge_sort_tanks_health(const std::vector<Tank>& original, int low , int high )
{
	if (original.size() > 1) {
		int mid = original.size() / 2;
		//C++ Equivalent to using Python Slices
		vector<int> lefthalf(original.begin(), original.begin() + mid);
		vector<int> righthalf(original.begin() + mid, original.begin() + original.size());
...
void Tmpl8::Game::merge(const std::vector<Tank>& original, int low, int high, int mid)
{
