...
of pivot */
int partition(vector<Tank&> original, int low, int high)
{
	int pivot = original[high].health; // pivot  
	int i = (low - 1); // Index of smaller element  

	for (int j = low; j <= high - 1; j++)
	{
		// If current element is smaller than the pivot  
		if (original[j].health < pivot)
		{
			i++; // increment index of smaller element  
			swap(&original[i], &original[j]);
		}
	}
	swap(&original[i + 1], &original[high]);
	return (i + 1);
}
...
vector<Tank*> Tmpl8::Game::quick_sort_tanks_health(const std::vector<Tank>& original, vector<Tank*> sorted_tanks, int low , int high )
{
	if (low < high)
	{
		/* pi is partitioning index, arr[p] is now
		at right place */
		int pi = partition(original, low, high);

		// Separately sort elements before  
		// partition and after partition  
		quick_sort_tanks_health(original, sorted_tanks,low, pi - 1);
		quick_sort_tanks_health(original, sorted_tanks ,pi + 1, high);
	}
}
