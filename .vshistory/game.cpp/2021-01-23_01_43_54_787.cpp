...
void Game::update(float deltaTime)
{
	...
	thread C(thread(std::thread([&] {collisionTanks; })));
	thread S(thread(std::thread([&] {smokeFunc; })));
	thread R(thread(std::thread([&] {rocketsFunc; })));
	thread P(thread(std::thread([&] {particleBeam; })));
	thread E(thread(std::thread([&] {explosionsFunc; })));
	collisionTanks();
	smokeFunc();
	rocketsFunc();
	particleBeam();
