...
{
	future<void> futRest = pool.enqueue([&] {	smokeFunc();
	explosionsFunc();
	particleBeam(); });
...
void Game::draw()
{
	...
	future<void> futDrawRockets = pool.enqueue([&] {drawRockets(); });
	future<void> futDrawSmokes = pool.enqueue([&] {drawSmokes(); });

	future<void> futDrawBeamsExplosions = pool.enqueue([&] {drawBeams(); drawExplosions(); });
	//future<void> futDrawExplosions = pool.enqueue([&]  });

	future<void> futDrawHB = pool.enqueue([&] {drawHealthBar(); });
	//drawRockets();
	//drawSmokes();
	//drawBeams();
	//drawExplosions();
	...
	futDrawRockets.wait();
	futDrawSmokes.wait();
	futDrawBeamsExplosions.wait();
	futDrawHB.wait();

	//futDrawExplosions.wait();
