...
vector<Tank*> Tmpl8::Game::quick_sort_tanks_health(const std::vector<Tank>& original, vector<Tank*> sorted_tanks, int l , int u )
{
	int j;
	if (l < u) {
		j = partition(original, l, u);
		quicksort(a, l, j - 1);
		quicksort(a, j + 1, u);
	}
}

int Tmpl8::Game::partition(vector<Tank&> tanks, int l, int u)
{
	int v, i, j, temp;
	v = tanks[l].health;
	i = l;
