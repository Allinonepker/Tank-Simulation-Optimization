...
//thread E(thread(&Game::explosionsFunc, this));
	...
	//future<void> fut = pool.enqueue([&] {collisionTanks(); });
	...
	//		for (it = rockets.begin(); it != rockets.end(); ) {
	//			if ((*it).position.x < 0 || (*it).position.x > SCRWIDTH || (*it).position.y < 0 || (*it).position.y > SCRHEIGHT || !(*it).active) {
	//				it = rockets.erase(it);
	//			}
	//			else {
	//				++it;
