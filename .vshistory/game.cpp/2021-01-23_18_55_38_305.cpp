...
void Game::init()
{
	...
	particle_beams.push_back(Particle_beam(vec2(SCRWIDTH / 2, SCRHEIGHT / 2), vec2(100, 100), &particle_beam_sprite, PARTICLE_BEAM_HIT_VALUE));
	particle_beams.push_back(Particle_beam(vec2(80, 80), vec2(100, 100), &particle_beam_sprite, PARTICLE_BEAM_HIT_VALUE));
	particle_beams.push_back(Particle_beam(vec2(1200, 600), vec2(100, 100), &particle_beam_sprite, PARTICLE_BEAM_HIT_VALUE));
}
...
void Game::rocketsFunc() {
	for (Rocket& rocket : rockets)
	{
		...
		rockets.erase(std::remove_if(rockets.begin(), rockets.end(), [](const Rocket& rocket) { return (!rocket.active || rocket.position.x < 0 || rocket.position.x > SCRWIDTH || rocket.position.y > SCRHEIGHT); }), rockets.end());
...
void Game::update(float deltaTime)
{
	...
	future<void> futCol = pool.enqueue([&] {collisionTanks(); });
	future<void> futRock = pool.enqueue([&] {rocketsFunc(); });
	{Timer timer;
		smokeFunc();
		explosionsFunc();
		particleBeam();
...
void Game::draw()
{
	...
	for (int t = 0; t < 2; t++)
	{
		...
		quicksort(tank_pointers_health, begin, begin + NUM_TANKS - 1, 0);
...
void Tmpl8::Game::quicksort(vector<Tank*>& values, int left, int right, int depth) {
	if (left < right) {
		int pivotIndex = partition(values, left, right);
		//if (pow(2, depth) <= 4) {
		//	future<void> fut = pool.enqueue([&] {quicksort(values, left, pivotIndex - 1, depth + 1); });
		//	/*quicksort(values, left, pivotIndex - 1, depth + 1);*/
		//	quicksort(values, pivotIndex, right, depth + 1);
		//	fut.wait();
		//}
		//else {
		//	quicksort(values, left, pivotIndex - 1, depth);
		//	quicksort(values, pivotIndex, right, depth);
		//}
