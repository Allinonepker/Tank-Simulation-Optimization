...
void merge(vector<Tank> tankos, int left, int right)
{
	int mid = (left + right) / 2;
	const int lsize = mid - left + 1, const rsize = right - mid;
	int pos = left;
	Tank leftarr[lsize];
	Tank rightarr[rsize];
	...
	for (int i = 0; i < lsize; i++)
		leftarr[i] = tankos[left + i];
	for (int i = 0; i < rsize; i++)
		rightarr[i] = tankos[mid + 1 + i];
	...
	while (i < lsize && j < rsize)
	{
		if (leftarr[i] <= rightarr[j])
			tankos[pos++] = leftarr[i++];
		else
			tankos[pos++] = rightarr[j++];
	}
...
/*
	...
	sorted halves
*/
void mergeSort(vector<Tank> tankos, int left, int right)
{
	// The following if condition is the base case. 
	if (left == right)
		return;
	int mid = (left + right) / 2;
	mergeSort(tankos, left, mid);
	mergeSort(tankos, mid + 1, right);
	merge(tankos, left, right);
}
