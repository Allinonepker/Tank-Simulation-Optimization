...
void Game::update(float deltaTime)
{
	...
	sort(tanks.begin(), tanks.end(), [](const Tank& lhs, const Tank& rhs) {
	...
	for (int i = 0; i < tanks.size(); i ++)
	{
		...
		{
			vec2 dir = tanks[i].get_position() - tanks[j].get_position();
			float dir_squared_len = dir.sqr_length();

			float col_squared_len = (tanks[i].get_collision_radius() + tanks[j].get_collision_radius());
			col_squared_len *= col_squared_len;

			if (dir_squared_len < col_squared_len)
			{
				tanks[i].push(dir.normalized(), 1.f);
			}
		{
	}
