...
void Game::update(float deltaTime)
{
	...
	vector<tuple<int, int, int, int>> gridsToCheck;
	...
	for (int i = 0; i < SCRWIDTH / COLSIZE; i++) {
		for (int j = 0; j < SCRHEIGHT / ROWSIZE; j++) {
			...
			(i == 0) ? x_bound_left = 0, x_bound_right = i + 1 : (i == SCRWIDTH / COLSIZE - 1) ? x_bound_right = i, x_bound_left = i - 1 : x_bound_left = i - 1,
				x_bound_right = i + 1;
			(j == 0) ? y_bound_bottom = 0, y_bound_top = j + 1 : (j == SCRHEIGHT / ROWSIZE - 1) ? y_bound_top = j, y_bound_bottom = j - 1 : y_bound_bottom = j - 1,
				y_bound_top = j + 1;
			gridsToCheck.push_back(make_tuple(x_bound_left, y_bound_bottom, x_bound_right, y_bound_top));
		}
	}

	for (tuple<int, int, int, int> tuple_grids : gridsToCheck) {
		for (int i = get<0>(tuple_grids); i <= get<2>(tuple_grids); i++) {
			for (int j = get<1>(tuple_grids); j <= get<3>(tuple_grids); j++) {
		...
		for (size_t j = 0; j < SCRHEIGHT / ROWSIZE; j++) {
			...
			for (it = tank_grid[i][j].tanks.begin(); it != tank_grid[i][j].tanks.end(); ) {
				...
				if ((*it)->xGrid != xGrid || (*it)->yGrid != yGrid) {
					...
					tank_grid[xGrid][yGrid].addTank(*it);
					it = tank_grid[i][j].tanks.erase(it);
				}
				else {
					++it;
...
void Game::checkColors() {
	...
	for (int i = 0; i < SCRWIDTH / COLSIZE; i++) {
		for (int j = 0; j < SCRHEIGHT / ROWSIZE; j++) {
			...
			if (!red)
				tank_grid[i][j].hasRed = false;
			else {
				tank_grid[i][j].hasRed = true;
				redGrids.push_back(make_pair(i, j));
			}

			if (!blue)
				tank_grid[i][j].hasBlue = false;
			else {
				tank_grid[i][j].hasBlue = true;
