...
void Game::draw()
{
	...
	vector<Tank> tanktemp = tanks;

	{
		Timer timer;

		quicksort(tankstemppoint, 0, tankstemppoint.size() - 1);
		//quicksort(tanktemp, 0, tanktemp.size() - 1);
	}
	//Draw sorted health bars
	for (int t = 0; t < 2; t++)
	{
		for (int i = 0; i < tankstemppoint.size(); i++)
		{
			...
			//screen->bar(health_bar_start_x, health_bar_start_y + (int)((double)HEALTH_BAR_HEIGHT * (1 - ((double)tanktemp.at(i).health / (double)TANK_MAX_HEALTH))), health_bar_end_x, health_bar_end_y, GREENMASK);
			screen->bar(health_bar_start_x, health_bar_start_y + (int)((double)HEALTH_BAR_HEIGHT * (1 - ((double)tankstemppoint.at(i)->health / (double)TANK_MAX_HEALTH))), health_bar_end_x, health_bar_end_y, GREENMASK);
		}
	}
}
